...
const MerzeApp = {
      ...
      <div class="main-container" style="padding-top:2.5rem;">
        <div style="background:var(--bg-surface); border:1px solid var(--border-subtle); border-radius:var(--radius-lg); padding:2rem; display:flex; gap:2rem; align-items:center; flex-wrap:wrap;">
          ...
          <div style="display:flex; gap:0.6rem; flex-wrap:wrap;">
            <button class="btn btn-secondary" onclick="MerzeApp.openEditProfileModal()">دەستکاریکردنی پڕۆفایل</button>
            ${!isVip ? `
  ...
  openEditProfileModal() {
    ...
    this.openModal(`
      <div class="modal-header">
        <h3 class="modal-title">دەستکاریکردنی زانیارییەکانی هەژمار</h3>
        <button class="player-btn" onclick="MerzeApp.closeModal()">✕</button>
      </div>
      <div class="modal-body">
        <div class="form-group">
        ...
        <div class="form-group">
          <label class="form-label">بەستەری وێنەی پڕۆفایل (Avatar URL):</label>
          <input type="text" id="prof-avatar" class="form-input" value="${user.avatar_url || ''}">
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary" onclick="MerzeApp.closeModal()">پاشگەزبوونەوە</button>
        <button class="btn btn-primary" onclick="MerzeApp.submitEditProfile()">پاشەکەوتکردن</button>
      </div>
    `);
  },
  ...
  async submitEditProfile() {
    ...
    if (!username) {
      alert("تکایە ناوی بەکارهێنەر بنووسە.");
      return;
    }
    const user = window.MerzeAuth.getUser();
    await window.MerzeDB.updateUser(user.id, { username, avatar_url });
    this.closeModal();
    this.showToast("زانیارییەکانی هەژمارەکەت لە داتابەیس نوێکرایەوە!", "success");
    this.renderCurrentRoute();
  }
};
