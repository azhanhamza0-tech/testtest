...
const MerzeDB = {
  ...
  async restGetCollection(collectionName) {
    const url = `https://firestore.googleapis.com/v1/projects/${this.PROJECT_ID}/databases/(default)/documents/${collectionName}?key=${this.API_KEY}&pageSize=100&_t=${Date.now()}`;
    try {
  ...
  // REST API: GET Single Document
  async restGetDocument(collectionName, docId) {
    const cleanId = encodeURIComponent(docId);
    const url = `https://firestore.googleapis.com/v1/projects/${this.PROJECT_ID}/databases/(default)/documents/${collectionName}/${cleanId}?key=${this.API_KEY}&_t=${Date.now()}`;
    try {
      const response = await fetch(url, { cache: 'no-store' });
      if (!response.ok) return null;
      const data = await response.json();
      return { id: docId, ...this.fromFirestoreFields(data.fields) };
    } catch (err) {
      return null;
    }
  },
  ...
  async init() {
    ...
    // 2. Fetch live movies from Firestore REST; if empty, seed immediately!
    ...
    // 3. Fetch live series from Firestore REST; if empty, seed immediately!
    const remoteSeries = await this.restGetCollection("series");
    if (remoteSeries !== null) {
      if (remoteSeries.length === 0) {
        console.log("🌱 چاندنی یەکەمجاری داتای زنجیرەکان لە Firestore...");
        for (const s of window.MerzeSeedData.series) {
          await this.restSetDocument("series", s.id, s);
        }
        this.setLocal(this.KEYS.SERIES, window.MerzeSeedData.series);
      } else {
        this.setLocal(this.KEYS.SERIES, remoteSeries);
      }
    }

    // 4. Fetch live payment settings from Firestore
  ...
  async deleteMovie(id) {
    ...
    // 1. Update local cache immediately so UI reflects deletion at once
    let current = this.getLocal(this.KEYS.MOVIES, []);
    current = current.filter(m => m.id !== id && m.slug !== id);
    this.setLocal(this.KEYS.MOVIES, current);

    // 2. Delete from Firestore via REST API
    const ok = await this.restDeleteDocument("movies", id);

    // 3. Also delete via SDK
    const db = this.getFirestore();
    if (db) {
  ...
  async updateSeries(id, updateData) {
    ...
    await this.restSetDocument("series", id, updated);

    const db = this.getFirestore();
    if (db) {
      try { await db.collection("series").doc(id).set(updated, { merge: true }); } catch (e) {}
    }
  ...
  async addEpisode(seriesId, seasonNumber, episodeData) {
    const series = await this.getSeriesById(seriesId);
    if (!series) return null;

    if (!series.seasons) series.seasons = [];
    let season = series.seasons.find(s => s.season_number === Number(seasonNumber));
    if (!season) {
      season = { season_number: Number(seasonNumber), title: `وەرزی ${seasonNumber}`, episodes: [] };
      series.seasons.push(season);
    }
    if (!season.episodes) season.episodes = [];

    const epNum = episodeData.episode_number || (season.episodes.length + 1);
    const newEp = {
      episode_number: Number(epNum),
      title: episodeData.title || `ئەڵقەی ${epNum}`,
      description: episodeData.description || '',
      thumbnail_url: episodeData.thumbnail_url || series.poster_url,
      video_url: episodeData.video_url || 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
      duration: episodeData.duration || '45m',
      release_date: episodeData.release_date || new Date().toISOString().split('T')[0]
    };

    season.episodes.push(newEp);
    return await this.updateSeries(seriesId, { seasons: series.seasons });
  },

  async deleteEpisode(seriesId, seasonNumber, episodeNumber) {
    const series = await this.getSeriesById(seriesId);
    if (!series || !series.seasons) return null;

    const season = series.seasons.find(s => s.season_number === Number(seasonNumber));
    if (!season || !season.episodes) return null;

    season.episodes = season.episodes.filter(ep => ep.episode_number !== Number(episodeNumber));
    return await this.updateSeries(seriesId, { seasons: series.seasons });
  },
  ...
  // --------------------------------------------------------------------------
  async getUsers() {
    const remote = await this.restGetCollection("users");
    if (remote !== null) {
      this.setLocal(this.KEYS.USERS, remote);
      return remote;
    }
    return this.getLocal(this.KEYS.USERS, []);
  },

  async updateUser(userId, updateData) {
    const users = await this.getUsers();
    const existing = users.find(u => u.id === userId) || {};
    const updated = { ...existing, ...updateData, id: userId, updated_at: new Date().toISOString() };

    // 1. Update in Firestore via REST API
    await this.restSetDocument("users", userId, updated);

    // 2. Also try SDK
    const db = this.getFirestore();
    if (db) {
      try { await db.collection("users").doc(userId).set(updated, { merge: true }); } catch (e) {}
    }

    // 3. Update local cache
    const current = this.getLocal(this.KEYS.USERS, []);
    ...
    this.setLocal(this.KEYS.USERS, current);

    // If this is currently logged in user, refresh their session
    const cur = window.MerzeAuth ? window.MerzeAuth.getUser() : null;
    if (cur && cur.id === userId) {
      window.MerzeAuth.saveUser({ ...cur, ...updated });
    }
  ...
  async grantVip(userId, days = 30) {
    let expireDateIso = null;
    if (days === 'lifetime' || Number(days) >= 9999) {
      expireDateIso = "2099-12-31T23:59:59.000Z";
    } else {
      const expDate = new Date();
      expDate.setDate(expDate.getDate() + Number(days));
      expireDateIso = expDate.toISOString();
    }

    const updates = {
      account_type: "VIP",
      vip_status: "active",
      vip_expires_at: expireDateIso
    };
  ...
  async deleteUser(userId) {
    await this.restDeleteDocument("users", userId);
    ...
    let current = this.getLocal(this.KEYS.USERS, []);
